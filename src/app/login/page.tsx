...
/**
 * Cuira Login (Phase 6 Demo).
 *
 * Drei-Rollen-Login als Split-Screen-Layout. Brand-Pane links (cuira-deep
 * mit Radial-Gradient), Form-Pane rechts (white).
 *
 * In Production: drei separate Routes /login/berater, /login/affiliate,
 * /login/kunde. Hier ein Tab-Switcher als Demo bevor Auth-Backend kommt.
 *
...
export default function LoginPage() {
  const [role, setRole] = useState<Role>("berater");
  const r = ROLES[role];
  ...
  return (
    <main className="grid min-h-screen md:grid-cols-[1fr_520px]">
      ...
      <div className="flex flex-col justify-center bg-white px-8 py-10 md:px-12">
        {/* Top: Help-Link */}
        <div className="mb-8 flex justify-end text-[12px]">
          <Link
            href="#"
        ...
        {/* Role-Tab Switcher (nur in Demo — Production = 3 Routes) */}
        <div
        ...
        >
          {(Object.keys(ROLES) as Role[]).map((key) => (
            <button
              ...
              style={{
                background: role === key ? "var(--surface)" : "transparent",
