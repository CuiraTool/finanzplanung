...
console.log("// Per-Kanton calibrated capital-payment tax breakpoints (Phase 3, 2026).");
...
console.log("");
console.log("export const KAPITAL_CALIBRATION_2026: Record<string, Array<{ kapital: number; einfache: number }>> = {");
...
for (const [k, rows] of sorted) {
  ...
  const factor = findFactor(info.cantonId, 2026, info.bfsIdHauptort);
  if (!factor) {
